...
export default class Register extends Component {
  ...
  handlerSubmit = async (e) => {
    e.preventDefault();
    if (this.state.name && this.state.email && this.state.password1 &&this.state.username) {
...
const Register = () => {
  ...
  const { email, name, password1, password2, username } = formData;
  //handler change from inputs
  const handlerChange = (text) => (e) => {
    setFormData({ ...formData, [text]: e.target.value });
  };
  const handlerSubmit = async (e) => {
    e.preventDefault();
    if (name && email && password1 && username) {
